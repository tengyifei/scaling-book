...
Within `{1,0:T(2,2)}`, the `1,0` part tells us the ordering of array dimensions in physical memory, from most minor to most major. You can read this part backwards and pick out the corresponding dimension in `f32[3,5]` to figure out that `5` is the major dimension and `3` is the minor dimension in physical memory.
